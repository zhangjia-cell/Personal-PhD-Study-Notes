...
\begin{lstlisting}[language=python,caption={基础环境的配置},label=code:environment configuration]
	sudo apt update && sudo apt install screen tree vim git htop gcc g++ colordiff python-is-python3 jq -y
	mkdir -p ~/miniconda3
	...
	conda create -y -n envname python=3.9.7
\end{lstlisting}

\textcolor{red}{注：第2~8行可以合在一起复制给(Linux)终端。}
