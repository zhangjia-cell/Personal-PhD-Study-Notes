...
\renewcommand*{\maketitle}{
	\begin{titlepage}
		...
		\parindent=0pt
		\includegraphics[scale=0.44]{Images/Cover_images/7_2.jpg}
		\vfill
		\begin{center}
...
\lstdefinelanguage{Markdown}{
	...
	alsoletter={\#-},
	% 标题
	morecomment=[l][\color{second}]{\#},
	% 引用
	morecomment=[l][\color{second}]{>},
	% 列表项
	morecomment=[l][\color{third}]{-},
	% 粗体 **bold**
...
% 自定义 Markdown 代码环境：
% 使用方式：
% \begin{markdowncode}[caption={示例},label={lst:md-example}]
	% # 标题
	% 一些 *斜体* 和 **粗体**，以及 `inline code`
	% \end{markdowncode}
\lstnewenvironment{markdowncode}[1][]{
	\lstset{style=markdownstyle,#1}
}{}
...
\usepackage{multirow} % 用于合并单元格等操作，若不需要可删除
...
% ------------------------------------------------------------%
% 置放章节
\input{Chapters/chapter1_Mathtool}
\newpage
\input{Chapters/chapter2_Theoretical_Analysis}
\newpage
\input{Chapters/chapter3_AIcode}
\newpage
\input{Chapters/chapter4_AI_Knowledge_Compilation}
\newpage
\input{Chapters/chapter5_Article_Summary}
\newpage
\input{Chapters/chapter6_Technical_Summary}
\newpage
\input{Chapters/chapter7_Project_Exercise}
\newpage
